...
```
The mean, rounded to one decimal point, is `r round(mean_steps,1)`, and the median is `r med_steps`.
...
Obtained results suggest that, on average, the maximum number of step is taken within the `r as.integer(names(mean_int_steps)[mean_int_steps==max(mean_int_steps)])` interval.

## Inputing missing values
The `steps` column of the dataset contains `r length(which(is.na(ACT$steps)))` missing values, encoded as `NA`. As missing values can affect the analysis of the data, we fill them in with the average number of steps for the respective five-minute intervals.
```{r}
